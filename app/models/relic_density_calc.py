...
def complex_quadrature(func, a, b, **kwargs):
    ...
    result_cq = (real_integral[0] + 1j*imag_integral[0],
...
def sigmav_res(T, s, mq, Mmed, mx, gr, gl, gx, dmname, Nf):
    ...
    TotalDecay = sig0.GM(dmname).GMdecay(s, mq, Mmed, mx, gr, gl, gx, Nf) + \
        sig0.GM('SM').GMdecay(s, mq, Mmed, mx, gr, gl, gx, Nf)

    epsR = ((Mmed**2) - (4*mx**2))/(4*mx**2)
    gamma_r = (Mmed * TotalDecay) / (4*mx**2)

    Bi = sig0.GM(dmname).GMdecay(
        s, mq, Mmed, mx, gr, gl, gx, Nf) / (TotalDecay)
    ...
    def Flzrx(T):
        ...
        result_flzrx = np.real(
            complex((1j/(np.pi)) * complex_quadrature_fortran(Flzrx_U, 0, 1, T)))
        return result_flzrx
...
def sigmav_gondolo(T, s, mq, Mmed, mx, gr, gl, gx, dmname, Bi, l, Nf):
    ...
    def Flzrx(T, l):
        ...
        errorel = 0.00001
        intlimit = 100
        result_flzrx = np.real(complex((1j/(np.pi)) * complex_quadrature(
            Flzrx_U, 0, 1, args=(T,), limit=intlimit, epsabs=erroabs, epsrel=errorel)[0]))
...
def naive_sigmav(T, s, mq, Mmed, mx, gr, gl, gx, dmname, Nf):
    ...
    def i_argument(s):
        integrand = sig0.SFV(dmname).sig0(s, mq, Mmed, mx, gr, gl, gx, Nf) * \
            np.sqrt(s) * (s - 4*mx**2) * sp.kn(1, np.sqrt(s)/T)
        return integrand
    ...
    integral_res = spquad(i_argument, 4*mx**2, np.inf,
                          limit=intlimit, epsabs=erroabs, epsrel=errorel)[0]
...
def naive_Ohm_numerical(s, mq, Mmed, mx, gr, gl, gx, dmname, Nf):
    if mx > Mmed/2:
        ...
        integral = spquad(naive_sigmav, T0, mx/xf, args=(s, mq, Mmed, mx, gr,
                          gl, gx, dmname, Nf), limit=intlimit, epsabs=erroabs, epsrel=errorel)[0]
...
def super_naive(s, mq, Mmed, mx, gr, gl, gx, dmname, Nf):
    ...
    kfact = (0.005)
    a = (kfact * sig0.SFV(dmname+".v").sig0(s, mq,
         Mmed, mx, gr, gl, gx, Nf) * 10**6)  # GeV^-2
    b = 0
    denom = a + ((3*b) / xf)
